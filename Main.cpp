...
int main(int argc, char *argv[]) {
	...
	int codec = static_cast<int>(capVideo.get(CV_CAP_PROP_FOURCC));
	...
	if (MANUAL_ROI > 0) {
		region_of_interest = cv::Rect(200,100,800,550);
	} else {
...
void matchCurrentFrameBlobsToExistingBlobs(std::vector<Blob> &existingBlobs, std::vector<Blob> &currentFrameBlobs) {
	...
	for (unsigned int i = 0; i < existingBlobs.size(); i++) {
		...
		if (existingBlobs[i].intNumOfConsecutiveFramesWithoutAMatch >=blob_persistency) {
